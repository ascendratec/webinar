  ...
  Alpine.data('eventsPage', () => ({
    ...
    selectFile(event) {
      ...
      if (!allowedTypes.includes(file.type)) {
        this.uploadError = 'Invalid file type. Accepted formats: PDF, PNG, JPEG';
        this.uploadFile = null;
        return;
      }

      if (file.size > maxSize) {
        this.uploadError = 'File exceeds maximum size of 5 MB';
        this.uploadFile = null;
        return;
      }
    ...
    async uploadDocument() {
      ...
      try {
        ...
        this.uploadError = err.message || 'Failed to get upload URL';
  ...
  Alpine.data('adminPage', () => ({
    ...
    async rejectRegistration(id) {
      ...
      if (reason.length < 1 || reason.length > 500) {
        this.$store.notification.show('Rejection reason must be between 1 and 500 characters', 'error');
        return;
      }
      this.actionLoading = true;
      try {
        ...
        this.$store.notification.show(err.message || 'Failed to reject registration', 'error');
      } finally {
        this.actionLoading = false;
      }
    },
    ...
    async createEvent() {
      ...
      try {
        ...
        this.$store.notification.show('Event created successfully!', 'success');
    ...
    async deleteEvent(eventId) {
      ...
      try {
        ...
        this.$store.notification.show('Event deleted successfully!', 'success');
      } catch (err) {
        this.$store.notification.show(err.message || 'Failed to delete event', 'error');
      } finally {
