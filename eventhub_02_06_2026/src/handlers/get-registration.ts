...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    logger.info('Buscando inscrição por ID', { id });
    ...
    logger.info('Inscrição encontrada com sucesso', { id, status: registration.status });

    return success(200, registration);
  } catch (err) {
    if (err instanceof AppError && err.isOperational) {
      logger.warn('Erro operacional ao buscar inscrição', { correlationId, error: err.message });
      return error(err.statusCode, err.message, correlationId);
    }

    logger.error('Erro interno ao buscar inscrição', { correlationId, error: err, stack: (err as Error).stack });
