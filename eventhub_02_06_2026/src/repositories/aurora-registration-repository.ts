...
export class AuroraRegistrationRepository implements RegistrationRepository {
  ...
  private async traceQuery<T>(operation: string, fn: () => Promise<T>): Promise<T> {
    const subsegment = tracer.getSegment()?.addNewSubsegment(`Aurora: ${operation}`);
    try {
  ...
  async createParticipantAndRegistration(
    ...
    return this.traceQuery('createParticipantAndRegistration', async () => {
  ...
  async getRegistrationById(id: string): Promise<Registration | null> {
    return this.traceQuery('getRegistrationById', async () => {
  ...
  async getRegistrationWithParticipant(
    ...
    return this.traceQuery('getRegistrationWithParticipant', async () => {
  ...
  async listAllRegistrations(): Promise<Registration[]> {
    return this.traceQuery('listAllRegistrations', async () => {
      const result = await (await this.getPool()).query<RegistrationRow>(
        `SELECT id, participant_id, event_id, status, document_s3_key, rejection_reason, created_at, updated_at
         FROM registrations
      ...
      return result.rows.map((row: RegistrationRow) => this.mapToRegistration(row));
    });
  }
  ...
  async updateStatus(
    ...
    return this.traceQuery('updateStatus', async () => {
      const result = await (await this.getPool()).query<RegistrationRow>(
  ...
  async findByEmailAndEvent(
    ...
    return this.traceQuery('findByEmailAndEvent', async () => {
      const result = await (await this.getPool()).query<RegistrationRow>(
  ...
  async createAuditLog(
    ...
    return this.traceQuery('createAuditLog', async () => {
  ...
  async deleteRegistration(registrationId: string): Promise<void> {
    return this.traceQuery('deleteRegistration', async () => {
      let client: PoolClient | undefined;
      ...
      try {
        ...
        await client.query(
          `DELETE FROM registration_audit_logs WHERE registration_id = $1`,
          [registrationId]
        );
